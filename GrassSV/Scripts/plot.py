...
series2   = ['Deletions', 'Inversions', 'Duplications', 'Insertions', 'BND',
            '(Misclassified as)\nDeletions', '(Misclassified as)\nInversions', '(Misclassified as)\nDuplications', '(Misclassified as)\nInsertions', '(Misclassified as)\nTranslocations',
            'False Negative']
series = series1 + series2

style_scheme = {     
    'Deletions':                            {'color': '#de4035', 'hatch': None , 'edgecolor': '#f5b0ab',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    'Inversions':                           {'color': '#fce54e', 'hatch': None , 'edgecolor': '#fff5cc',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    'Duplications':                         {'color': '#2c83db', 'hatch': None , 'edgecolor': '#a8c6f3',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    'Insertions':                           {'color': '#58db2c', 'hatch': None , 'edgecolor': '#b3ec9f',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    'Translocations':                       {'color': '#be2cdb', 'hatch': None , 'edgecolor': '#e6b3f2',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    'BND':                                  {'color': '#a36424', 'hatch': None , 'edgecolor': '#d6b999',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    'False Negative':                       {'color': '#333333', 'hatch': None , 'edgecolor': '#999999',     'textcolor' : 'white', 'textformat' : '', 'textposition' : -10,    'textalways' : True},
    '(Misclassified as)\nDeletions':        {'color': '#de4035', 'hatch': '\\\\\\', 'edgecolor': '#f5b0ab',  'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    '(Misclassified as)\nInversions':       {'color': '#fce54e', 'hatch': '\\\\\\', 'edgecolor': '#fff5cc',  'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    '(Misclassified as)\nDuplications':     {'color': '#2c83db', 'hatch': '\\\\\\', 'edgecolor': '#a8c6f3',  'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    '(Misclassified as)\nInsertions':       {'color': '#58db2c', 'hatch': '\\\\\\', 'edgecolor': '#b3ec9f',  'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    '(Misclassified as)\nTranslocations':   {'color': '#be2cdb', 'hatch': '\\\\\\', 'edgecolor': '#e6b3f2',  'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    '(Were)\nDeletions':                    {'color': '#de4035', 'hatch': '///', 'edgecolor': '#f5b0ab',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    '(Were)\nInversions':                   {'color': '#fce54e', 'hatch': '///', 'edgecolor': '#fff5cc',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    '(Were)\nDuplications':                 {'color': '#2c83db', 'hatch': '///', 'edgecolor': '#a8c6f3',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
    '(Were)\nInsertions':                   {'color': '#58db2c', 'hatch': '///', 'edgecolor': '#b3ec9f',     'textcolor' : 'black', 'textformat' : '', 'textposition' : -10,    'textalways' : False},
...
legend2 = {
    ...
    'False Positive':{'color': '#999999', 'hatch': '///' , 'edgecolor': '#333333'},
...
def extract_data2(dataset, entries):
    sv_types = ['DEL', 'INV', 'DUP', 'INS', 'TRN', 'BND']
    type_map = {'DEL': 'Deletions', 'INV': 'Inversions', 'DUP': 'Duplications', 'INS': 'Insertions', 'BND':'BND'}

    data = {'Deletions': {}, 'Inversions': {}, 'Duplications': {}, 'Insertions': {}}
    ...
    for entry in entries:
        ...
        entry_values = {'DEL' : entry.deletions, 'INV' : entry.inversions, 'DUP' : entry.duplications, 'INS' : entry.insertions}
        # pprint(entry)
        ...
        for entry_type, value in entry_values.items():
            ...
            if sanitized_value != 0:
                ...
                if dataset == label:
                    if label == sv_label or 'BND' == sv_label:
                        data[dataset][sv_label] = sanitize_func(value)
                    else:
                        data[dataset][f"(Misclassified as)\n{sv_label}"] = sanitize_func(value)

        # if sv_label != 'BND':
        #     data[sv_label]['False Positive'] += sanitize_func(entry.false_positive)
        data[dataset][f"(Were)\n{sv_label}"] = sanitize_func(entry.false_positive)

    for dictionary in data.values():
        # pprint(dictionary)
        dictionary['False Negative'] = 500 - sum([dictionary[key] for key in series2])
...
width = 0.08
spacing = 0.12
def draw_subplot(ax, series, caller_data_dict, x, bar_lower):
    # for dataset, caller_data_dict in dataset_data_dict.items():
    for i, caller in enumerate(caller_data_dict):
        ...
        for category in series:
            ...
            for bar in bars:
                yval = bar.get_height()
                if yval > 25 or style['textalways']:
                    if category != "False Positive":
                        ax.text(bar.get_x() + bar.get_width()/2, bar.get_y() + yval + bar_lower, f"{style['textformat']}{int(yval)}", va='center', ha='center', fontsize=8, rotation=0, color=style['textcolor'])
                    else:
                        ax.text(bar.get_x() + bar.get_width()/2, bar.get_y() + yval + style['textposition'], f"{style['textformat']}{int( bar.get_y())}", va='center', ha='center', fontsize=8, rotation=0, color=style['textcolor'])
...
def create_grouped_stacked_bar_diagram(data_dict, title):
    ...
    dataset_caller_data_dict = { dataset : {} for dataset in data_dict.keys()} 
    for dataset, dataset_data_dict in data_dict.items():
        caller_data_dict = dataset_data_dict
        for caller, data in dataset_data_dict.items():
            caller_data_dict[caller] = extract_data2(dataset, data)
            labels = caller_data_dict[caller].keys()
        caller_labels = dataset_data_dict.keys()
        dataset_caller_data_dict[dataset] = caller_data_dict

    # pprint(dataset_caller_data_dict)

    caller_data_dict
    for caller, caller_data in caller_data_dict.items():
        for sv_type in caller_data.keys():
            print(f"\n\n\t\t{sv_type}\t{caller}\n\n")
            pprint(dataset_caller_data_dict[sv_type][caller])
            caller_data_dict[caller][sv_type] = dataset_caller_data_dict[sv_type][caller][sv_type]


    x = np.arange(len(labels))
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), gridspec_kw={'height_ratios': [650, 500]}, sharex=True)
    draw_subplot(ax1, series1, caller_data_dict, x, -8)
    draw_subplot(ax2, series2, caller_data_dict, x, -20)
    ...
    for i, caller in enumerate(caller_labels):
        for j in range(len(labels)):
            ...
            ax2.text(positions, -120, caller, rotation=90, ha='center')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax2.set_xlabel('Separate Datasets with single SV type\n(500 SVs each)')
    ...
    plot_legend1 = create_legend(legend1, ax1, **{'ncol' : 6, 'bbox_to_anchor' :(0.5, 1.1), 'loc' : 'upper center'})
    ...
    for format in formats:
        plot_file = f"bar_diagram_plot_2.{format}"
...
def plot_bar_diagram():
    ...
    datasets = ['Inversions', 'Insertions', 'Deletions', 'Duplications' ]
    # Load the pickled collected_data for each caller
    collected_data = {dataset : {caller: load_pickled_data(f"fastadna_{dataset.lower()}/{file}") for caller, file in pickle_files.items()} for dataset in datasets}
